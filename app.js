...
const defaultStapleTargets=[
 {name:'Ground Beef',target:10,unit:'lbs',aliases:['ground beef','beef']},
 {name:'Milk',target:2,unit:'gallon',aliases:['milk']},
 {name:'Eggs',target:36,unit:'each',aliases:['eggs','egg']},
 {name:'Bread',target:2,unit:'loaf',aliases:['bread']},
 {name:'Chicken',target:8,unit:'lbs',aliases:['chicken']},
 {name:'Cheese',target:2,unit:'lbs',aliases:['cheese']},
 {name:'Butter',target:4,unit:'sticks',aliases:['butter']},
 {name:'Pasta',target:4,unit:'box',aliases:['pasta','spaghetti','noodles']},
 {name:'Rice',target:5,unit:'lbs',aliases:['rice']},
 {name:'Tortillas',target:2,unit:'pack',aliases:['tortillas','tortilla']}
];
const preferredStores=['Walmart Pine City','Walmart Cambridge','ALDI Pine City','ALDI Cambridge','Target Cambridge','Coborn’s Pine City','Jerry’s Foods North Branch','Costco Coon Rapids','Costco Woodbury'];
const mockDeals={
 ...
 'ALDI Pine City':['Milk, eggs, bread value picks','Produce and pantry basics'],
...
const valueRules=[
 {keys:['ground beef','beef'],unit:'lbs',price:4.49},
 {keys:['chicken'],unit:'lbs',price:3.49},
 {keys:['pork'],unit:'lbs',price:3.29},
 {keys:['milk'],unit:'gallon',price:3.25},
 {keys:['eggs','egg'],unit:'each',price:.22},
 {keys:['bread'],unit:'loaf',price:2.75},
 {keys:['cheese'],unit:'lbs',price:4.99},
 {keys:['butter'],unit:'sticks',price:1.10},
 {keys:['pasta','spaghetti','noodles'],unit:'box',price:1.65},
 {keys:['rice'],unit:'lbs',price:1.10},
 {keys:['tortilla'],unit:'pack',price:2.50}
];
...
const units=['item','each','count','lb','lbs','oz','g','kg','jar','can','box','bag','bottle','carton','package','pack','loaf','dozen','gallon','quart','pint','cup','tbsp','tsp','fl oz','stick','sticks'];
...
function init(){
 ...
 $('savePreferencesBtn').addEventListener('click',savePreferences);$('manualGroceryBtn').addEventListener('click',addManualGrocery);$('resetStaplesBtn').addEventListener('click',resetStapleTargets);
 updateScanHint();renderStapleTargetEditor();load();showTab('home');
}
...
function render(){renderInventory();renderGroceryList();renderHome()}
...
function renderGroceryList(){const low=items.filter(i=>i.low_stock).map(i=>({name:i.name,detail:i.quantity+' '+i.unit+' · '+i.location,source:'inventory',id:i.id}));const targetLow=stapleStats().filter(s=>s.level==='Low').map(s=>({name:s.name,detail:'Target gap: '+fmt(Math.max(s.target-s.current,0))+' '+s.unit,source:'target'}));const all=[...low,...targetLow,...groceryItems];const list=all.length?all.map((i,idx)=>'<div class="grocery-item"><strong>'+esc(i.name)+'</strong><span>'+esc(i.detail||'')+'</span>'+(i.source==='inventory'?'<button class="secondary small" onclick="toggleLow(\''+i.id+'\',false)">Got it</button>':i.source==='target'?'<button class="secondary small" onclick="addManualGroceryNamed(\''+escAttr(i.name)+'\')">Plan</button>':'<button class="secondary small" onclick="clearManualGrocery('+(idx-low.length-targetLow.length)+')">Remove</button>')+'</div>').join(''):"<p class='muted'>No grocery items yet.</p>";$('groceryList').innerHTML=list+shoppingPlanPreview(all)}
function getStapleTargets(){return JSON.parse(localStorage.getItem('pantrypal_staple_targets')||JSON.stringify(defaultStapleTargets))}
function saveStapleTargets(t){localStorage.setItem('pantrypal_staple_targets',JSON.stringify(t));renderStapleTargetEditor();renderHome()}
function resetStapleTargets(){saveStapleTargets(defaultStapleTargets);$('pantryScoreSummary').textContent='Staple targets reset to the Rush City starter list.'}
function stapleAmount(target){return items.reduce((sum,i)=>{const hay=String(i.name||'').toLowerCase();const match=(target.aliases||[target.name]).some(a=>hay.includes(String(a).toLowerCase()));return match?sum+convertQty(Number(i.quantity)||0,i.unit,target.unit):sum},0)}
function convertQty(q,from,to){const f=String(from||'').toLowerCase(),t=String(to||'').toLowerCase();if(t==='lbs'&&['lb','lbs','pound','pounds'].includes(f))return q;if(t==='gallon'&&['gallon','gallons'].includes(f))return q;if(t==='each'&&['each','count','item','dozen'].includes(f))return f==='dozen'?q*12:q;if(t==='loaf'&&['loaf','loaves'].includes(f))return q;if(t==='sticks'&&['stick','sticks'].includes(f))return q;if(t==='box'&&['box','boxes'].includes(f))return q;if(t==='pack'&&['pack','package'].includes(f))return q;return q}
function stapleLevel(ratio){if(ratio<.35)return'Low';if(ratio<.75)return'Medium';return'High'}
function stapleStats(){return getStapleTargets().map(t=>{const current=stapleAmount(t);const ratio=t.target?Math.min(current/t.target,1):0;return{...t,current,ratio,level:stapleLevel(ratio)}})}
function pantryScore(stats=stapleStats()){return stats.length?Math.round(stats.reduce((n,s)=>n+s.ratio,0)/stats.length*100):0}
function setTextIfPresent(id,value){const el=$(id);if(el)el.textContent=value}
function renderHome(){
 const stats=stapleStats();
 const score=pantryScore(stats);
 const low=stats.filter(s=>s.level==='Low');
 document.documentElement.style.setProperty('--score',score);
 setTextIfPresent('pantryScore',score);
 setTextIfPresent('pantryScorePill',score+'%');
 setTextIfPresent('pantryScoreSummary',score>=80?'Cabin shelves look strong.':score>=50?'Good start — a few staples need a store run.':'Low staple coverage — build a quick shopping plan.');
 setTextIfPresent('homeInventoryStat',items.length+' item'+(items.length===1?'':'s'));
 setTextIfPresent('homeLowStat',low.length+' low');
 const staplesEl=$('staplesGlance');
 if(staplesEl)staplesEl.innerHTML=stats.map(s=>'<div class="staple-card"><div class="item-head"><h3>'+esc(s.name)+'</h3><strong class="level-'+s.level.toLowerCase()+'">'+s.level+'</strong></div><p>'+fmt(s.current)+' / '+fmt(s.target)+' '+esc(s.unit)+'</p><div class="staple-meter" style="--pct:'+Math.round(s.ratio*100)+'%"><span></span></div></div>').join('');
 const criticalEl=$('criticalStaples');
 if(criticalEl)criticalEl.innerHTML=low.length?low.map(s=>'<div class="grocery-item"><strong>'+esc(s.name)+'</strong><span>Need about '+fmt(Math.max(s.target-s.current,0))+' '+esc(s.unit)+' to hit target.</span><button class="small" onclick="addManualGroceryNamed(\''+escAttr(s.name)+'\')">Add</button></div>').join(''):'<p class="muted">No critical low staples right now.</p>';
 renderPantryHealth(stats);renderPantryValue();renderTonightDinner(stats);renderStoreDeals(low)
}
function renderPantryHealth(stats){const groups=[{name:'Protein',score:categoryScore(['Meat','Beef','Chicken','Pork','Fish/Seafood','Deli Meat'])},{name:'Dairy',score:categoryScore(['Dairy','Eggs'])},{name:'Produce',score:categoryScore(['Produce'])},{name:'Frozen',score:locationScore(['Freezer','Garage Freezer','Small Freezer'])},{name:'Staples',score:pantryScore(stats)}];$('pantryHealth').innerHTML=groups.map(g=>'<div class="health-row"><strong>'+esc(g.name)+'</strong><span>'+g.score+'%</span><div class="health-meter" style="--pct:'+g.score+'%"><span></span></div></div>').join('')}
function categoryScore(cats){const count=items.filter(i=>cats.includes(i.category)).length;return Math.max(0,Math.min(100,Math.round(count/4*100)))}
function locationScore(locs){const count=items.filter(i=>locs.includes(i.location)).length;return Math.max(0,Math.min(100,Math.round(count/5*100)))}
function renderPantryValue(){
 const totals=items.reduce((acc,i)=>{const loc=i.location||'Pantry';const bucket=loc.toLowerCase().includes('freezer')?'Freezer':loc.toLowerCase().includes('fridge')?'Fridge':'Pantry';acc[bucket]=(acc[bucket]||0)+estimateItemValue(i);return acc},{Pantry:0,Fridge:0,Freezer:0});
 const total=Object.values(totals).reduce((a,b)=>a+b,0);
 const meals=Math.max(0,Math.round(total/6));
 setTextIfPresent('homeValueStat',money(total));
 setTextIfPresent('homeMealsStat',meals+' meals');
 const el=$('pantryValue');
 if(el)el.innerHTML='<div class="value-row"><span>Total estimated value</span><strong class="value-total">'+money(total)+'</strong></div>'+['Pantry','Fridge','Freezer'].map(k=>'<div class="value-row"><span>'+k+'</span><strong>'+money(totals[k]||0)+'</strong></div>').join('')+'<p class="muted">Estimated meals available: <strong>'+meals+'</strong>. Values are rough planning estimates, not live prices.</p>'
}
function estimateItemValue(item){const name=String(item.name||'').toLowerCase();const rule=valueRules.find(r=>r.keys.some(k=>name.includes(k)));if(rule)return convertQty(Number(item.quantity)||0,item.unit,rule.unit)*rule.price;const qty=Number(item.quantity)||1;if(item.category==='Meat'||item.category==='Beef'||item.category==='Chicken'||item.category==='Pork')return qty*4;if(item.category==='Produce')return qty*2.5;if(item.category==='Dairy'||item.category==='Eggs')return qty*3;if(item.location&&String(item.location).toLowerCase().includes('freezer'))return qty*4;return qty*2}
function money(n){return '$'+Number(n||0).toLocaleString(undefined,{maximumFractionDigits:0})}
function renderTonightDinner(stats){const pick=starterRecipes.map((r,idx)=>({r,idx,have:r.ingredients.filter(x=>findInventoryMatch(x)).length})).sort((a,b)=>b.have-a.have)[0];$('tonightDinner').innerHTML=pick?'<div class="recipe"><div class="item-head"><h3>'+esc(pick.r.name)+'</h3><span class="pill">Mild</span></div><p>Best built-in match from your pantry right now. Uses '+pick.have+' ingredient match(es).</p><button onclick="showTab(\'recipes\')">Open Recipes</button></div>':'<p class="muted">Add pantry items to get a dinner idea.</p>'}
function renderStoreDeals(low){
 const needs=(low.length?low:stapleStats().filter(s=>s.level!=='High')).slice(0,4).map(s=>s.name);
 const stores=preferredStores.slice(0,3);
 const el=$('storeDealsPreview');
 if(el)el.innerHTML=stores.map((store,idx)=>'<div class="staple-card store-card"><div class="item-head"><h3>'+esc(store)+'</h3><span class="pill">Mock</span></div><p class="muted">'+esc((mockDeals[store]||[]).join(' · '))+'</p><p><strong>Plan:</strong> '+esc(needs.length?needs.join(', '):'Watch weekly ad for stock-up pricing')+'</p></div>').join('')
}
function renderStapleTargetEditor(){const targets=getStapleTargets();$('stapleTargetEditor').innerHTML=targets.map((t,i)=>'<div class="target-row"><input id="target-name-'+i+'" value="'+esc(t.name)+'"><input id="target-qty-'+i+'" type="number" step="0.25" value="'+esc(t.target)+'"><input id="target-unit-'+i+'" value="'+esc(t.unit)+'"><button class="small secondary" onclick="saveTargetRow('+i+')">Save</button></div>').join('')}
function saveTargetRow(i){const targets=getStapleTargets();targets[i]={...targets[i],name:$('target-name-'+i).value.trim(),target:+$('target-qty-'+i).value||1,unit:$('target-unit-'+i).value.trim()||'item',aliases:[$('target-name-'+i).value.trim().toLowerCase()]};saveStapleTargets(targets)}
function shoppingPlanPreview(all){const names=all.map(i=>i.name).slice(0,6);const focus=names.length?names.join(', '):'weekly ad check';return '<div class="compact-card trip-card"><p class="eyebrow">Shopping Recommendation</p><h3>Trip Value Placeholder</h3><p class="muted">Future live pricing will compare savings, fuel, drive time, and bulk value before recommending extra stops.</p><div class="trip-value"><div><span>Est. savings</span><strong>Pending</strong></div><div><span>Extra miles</span><strong>Pending</strong></div><div><span>Worth trip?</span><strong>Needs prices</strong></div></div><p><strong>Plan focus:</strong> '+esc(focus)+'</p></div><div class="compact-card"><p class="eyebrow">Shopping Plan Preview</p><h3>Preferred Stores</h3>'+preferredStores.map((s,idx)=>'<div class="grocery-item"><strong>'+esc(s)+'</strong><span>Mock slot '+(idx+1)+': '+esc(names[idx%Math.max(names.length,1)]||'weekly ad check')+' · ready for future ad, price-history, and trip-value data.</span></div>').join('')+'</div>'}
function addManualGroceryNamed(name){if(!groceryItems.some(i=>i.name.toLowerCase()===String(name).toLowerCase()))groceryItems.push({name,detail:'Low staple target'});localStorage.setItem('pantrypal_grocery_items',JSON.stringify(groceryItems));renderGroceryList();showTab('grocery')}
